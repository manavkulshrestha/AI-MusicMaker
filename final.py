    ...
    for children in base_tree[i]:
        for child in children:
            ...
            if next_cf_delta_mag == 1:
                #step
                upperBound = next_cf_note.get_num()-child.note.get_num()
                possibilities_delta = range(-5,upperBound)
            else:
                #leap
                possibilities_delta = range(-1,2)

            new_children = []
            for delta in possibilities_delta:
                test = child.note.get_num()+delta
                if delta == -4:
                    print(1)
                new_base_note = num_to_note(child.note.get_num()+delta)
                boval = is_legal(next_cf_note, new_base_note)
                if boval != False:
                    if boval == None:
                        #allowed, pass
                        #not allowed, continue
                        cf_diff = cantus_firmus[i+1][0] # - means down
